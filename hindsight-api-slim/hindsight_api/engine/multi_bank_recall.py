...
3. Exact / normalized-text dedup (``dedup_exact_normalized``) — cheap v2, BEFORE token cut.
...
# Was hard-coded ``"none"`` in v1 (no cross-bank dedup). v2: exact/normalized text.
...
def dedup_exact_normalized(
    ...
    Returns ``(kept, dedup_dropped)`` where ``dedup_dropped`` is the number of
    facts removed as duplicates.
    """
    seen: set[str] = set()
...
def merge_cap_dedup_cut(
    ...
    *,
    ...
    Returns ``(final_facts, dedup_dropped)``. Orchestrators may call the steps
    individually instead; this helper encodes the required order.
    """
    ...
    deduped, dropped = dedup_exact_normalized(merged)
    return cut_to_token_budget(deduped, max_tokens), dropped
