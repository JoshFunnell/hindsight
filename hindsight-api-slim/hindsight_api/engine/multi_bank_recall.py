...
2. Per-bank pre-cap + union select (``select_union_candidates``) — floor then
   ...
   not O(N_banks). LAT1: per-bank CE was 94% of single-bank wall.
...
4. Per-bank floor after the CE cut (``apply_per_bank_floor``).
   This floor is BEFORE the token cut. It does not guarantee a bank
   survives the HTTP list when ``max_tokens`` is tight (P2-5).
...
- Union cap 200 (not 50/100): the 2026-08-10 50-vs-100 class still applies to facts
  that only CE would have promoted from outside each bank's pre-cap/union slice.
  U=200 keeps per-bank RRF ranks 0-39 at N=5 (matrix golds at 36/37).
...
# Union CE pool. 200 not 50/100/150: LAT4 measured matrix golds at per-bank
# RRF 36/37. Those survive a 5-bank interleave iff U >= 5*(37+1) = 190.
# Default 200 admits ranks 0-39 at N=5. Cost stays O(cap).
# Pre-CE cut (P2-4): with union_cap U and N banks that returned facts, interleave
# fill admits ~U/N per bank at RRF rank.
# Floor default 1 = before the token cut, do not drop a bank that returned
# facts when union_cap >= n_banks (true at defaults: 200 >= 10). The floor
# does not survive a tight token cut (P2-5).
DEFAULT_MULTI_UNION_CAP = 200
DEFAULT_PER_BANK_FLOOR = 1
# Per-bank recall_async still token-cuts in RRF order when union CE skips
# per-bank CE. Caller max_tokens=2000 (lt1b) kept only ~6-16 RRF-head facts
# and dropped the matrix golds (RRF 13-37) before union CE ran — LAT5
# CE-collapse. Subcall budget keeps the pre-cap (50) list intact; the
# caller's max_tokens is applied after union CE.
...
def _parse_iso_datetime(value: Any) -> datetime | None:
    ...
    LAT3 only caught ValueError. ``datetime.replace("Z", ...)`` is
    ``datetime.replace`` (year/month/...) and raises TypeError, which
    aborted the whole union CE. Strings from MemoryFact.isoformat() are
    unchanged.
    """
...
def union_per_bank_max_tokens(caller_max_tokens: int, *, union_ce: bool) -> int:
    ...
    RRF order — that cut is what dropped the LAT3 AFTER golds. The caller
    budget is still applied after union CE via merge_cap_dedup_cut.
    """
...
def ce_pair_from_retrieval(query: str, retrieval: RetrievalResult) -> tuple[str, str]:
    """Same document text as CrossEncoderReranker.rerank (image reranking.py)."""
    doc_text = retrieval.text or ""
...
def union_predict_pairs(query: str, facts: Sequence[MemoryFact]) -> list[tuple[str, str]]:
    ...
    for item in memory_facts_to_merged_candidates(facts):
        pairs.append(ce_pair_from_retrieval(query, item.candidate.retrieval))
    return pairs
...
def single_bank_predict_pairs(query: str, facts: Sequence[MemoryFact]) -> list[tuple[str, str]]:
    ...
    for fact in facts:
        ...
        pairs.append(ce_pair_from_retrieval(query, retrieval))
    return pairs
...
def select_union_candidates(
    ...
    *,
    ...
    With union_cap U and N non-empty banks the pre-CE cut admits ~U/N facts
    per bank at RRF rank (interleave, not first-bank biased). Example: U=100,
    N=5 -> ~20 per bank (RRF ranks 0-19). Rank 25 in one of those banks is
    lost unless U is raised (knob: multi_union_cap / HINDSIGHT_API_MULTI_UNION_CAP).
    """
...
def apply_per_bank_floor(
    ...
    *,
    ...
    provided ``keep >= n_banks_with_facts``. This is the cut BEFORE the token
    budget: ``merge_cap_dedup_cut`` may still drop that floor fact when
    ``max_tokens`` is smaller than the stronger hits. Narrowed claim: the
    floor never loses a bank *before the token cut*.
    """
    keep = max(0, int(keep))
...
def memory_facts_to_merged_candidates(
    facts: Sequence[MemoryFact],
) -> list[TokenedCandidate]:
...
def has_usable_reranker_scores(
    bank_results: Sequence[tuple[str, Sequence[MemoryFact]]],
) -> bool:
